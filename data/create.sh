...
# -----------------------
#  Определение метода подключения к MySQL (один раз)
# -----------------------
...
FLUSH PRIVILEGES;
"
  printf '%s' "$SQL_CLEANUP" | mysql $MYSQL_OPTS 2>/dev/null || true
...
if mysql $MYSQL_OPTS -e "SELECT SCHEMA_NAME FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME = '$DB_NAME'" | grep -q "$DB_NAME"; then
...
if mysql $MYSQL_OPTS -e "SELECT User FROM mysql.user WHERE User = '$DB_USER'" | grep -q "$DB_USER"; then
  echo "Ошибка: Пользователь MySQL $DB_USER уже существует."
  exit 1
fi
...
chown -R "$SITENAME":web "$WEBROOT"
chmod -R 2770 "$WEBROOT"
...
sudo -u "$SITENAME" bash -c "cat > \"$DOCROOT/index.php\" <<'PHP'
<?php
echo 'Test';
PHP"

chown "$SITENAME":www-data "$DOCROOT/index.php"
chmod 664 "$DOCROOT/index.php"
