...
class MessageParser
{
    ...
    private const PRICE_BARE_PATTERN = '/[-–—:=]?\s*(?<!\+)(\d{1,3}\s\d{3}|\d{2,6})\s*$/u';
    ...
    private const NOISE_NAME_PATTERN = '/^(?:только|лишь|либо|или|можно|нужно|если|все|всё|обмены?|торг|состав|рассмотрю|кланам|продам|продаю|куплю|покупаю|обменяю|меняю|услуги|пасс?ивки|экипировка|ресурсы|наличие)\b/ui';
    ...
    public function __construct()
    {
        ...
        $this->keywordMap = config('parser.keyword_map', [
            ...
            'trade'   => ['обменяю', 'меняю'],
            'service' => ['предлагаю услуги', 'услуги', 'выполню', 'найму'],
        ]);
    }
    ...
    public function extractPrice(string $line): array
    {
        ...
        // 6. Голое число в конце строки: "Товар - 4000", "Товар 650", "Товар - 6 900"
    ...
    private function normalizeFakeRomanGrade(string $line): string
    {
        ...
        // Заменяем | и строчную l на I внутри скобок грейда
        // Покрывает миксы: [IIl] → [III], [l|I] → [III], [Il] → [II]
        $line = preg_replace_callback('/\[([IVXliv|+\s]+)]/ui', function ($m) {
            $grade = str_replace(['|', 'l', 'L'], ['I', 'I', 'I'], $m[1]);
            return '[' . $grade . ']';
        }, $line);
    ...
    private function cleanName(string $name): string
    {
        ...
        // \b — не обрезаем начало слов: "Потрошитель", "Повар", "Подарок", "Отрава", "Закалка"
        $name = preg_replace('/^(?:(?:по|от|за)\b\s*[,.]?\s*)+/ui', '', $name);
